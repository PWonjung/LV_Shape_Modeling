...
from models.loss import lv_criterion
...
def train_data(data_file, tag=None, id=None):
    ...
    with open(data_file, "rb") as f:
        data = pickle.load(f)
    print("@@@@@!!",id)
    # print(glob.glob(f"/root/LV/LV_Shape_Modeling/ipynb/MICCAI-LV/results/{id}/2000_*.npy"))
    ...
    print(vert.shape)
    zero = np.zeros((500,3)).astype(np.float32)
    vert = np.concatenate([vert,zero])
    print(vert.shape)
    ...
    hippo_idx = np.setdiff1d(np.unique(hippo_tri), np.unique(lv_tri)).astype(int)
    print(hippo_idx)
    vert[hippo_idx]*=0    
...
def main(args):
    ...
    model.train()
    # data preparation
    vertices, lv_tri, hippo_tri, lv_target,hippo_target,lv_pred_mesh,hippo_pred_mesh = train_data(args.data_path, args.tag, id = args.sub_id)
    # Convert vertices and faces to tensors and move to GPU
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        loss, log = lv_criterion(verts, lv_target, lv_tri, args.lda)
        ...
        loss += (l2_lv_norm_loss) * args.lda[-1]

...
label = 'aibl_normal_tex_4'
def process_input(data_path, sub_id):
    command = f"python ../../optim/lv_train.py --data_path {data_path} --tag {label}  --learning_rate 0.0005 --sub_id {sub_id} --gpu 2 --lda 2 2 50 10 10 1 1"
...
def main():
    ...
    filelist = glob.glob(f"/root/LV/LV/2502LV/AIBL/normal_mid_tgt/*.pkl")
    filelist = filelist
    print(filelist)
    ...
    for i in range(len(filelist)//num_item+1):
        ...
        for fold in folders:
            ...
            data_path = rf"{fold}"
            exist5000 = glob.glob(f"/root/LV/LV_Shape_Modeling/ipynb/MICCAI-LV/results/{label}/out/{sub_id}/0smallest*.npy")

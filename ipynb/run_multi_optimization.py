...
def process_input(data_path, sub_id):
    command = f"python ../../voxel2mesh/hippo_lv_train2.py --data_path {data_path} --tag {label}  --learning_rate 0.0005 --sub_id {sub_id} --gpu 0 --lda 2 2 1500 500 100 1 1"
...
def main():
    ...
    for i in range(len(filelist)//num_item+1):
        ...
        for fold in folders:
            ...
            data_path = rf"{fold}"
            exist5000 = glob.glob(f"/root/LV/lv-parametric-modelling/ipynb/MICCAI-LV/results/{label}/out/{sub_id}/000*.npy")
